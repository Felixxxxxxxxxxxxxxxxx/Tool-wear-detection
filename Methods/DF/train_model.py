...
class data():
    ...
    def feature_analyse(self,i):

        self.choose_data(i)

        self.Pretrained_Weight_Path = f"/rwthfs/rz/cluster/home/de532237/cancer/Bachelorarbeit/WP2/Methods/feature_analyse/Model_{i-1}/checkpoint/model.ckpt"

        X_train_0, Y_train_0, X_val_0, Y_val_0 = self.X_train_0, self.Y_train_0, self.X_val_0, self.Y_val_0 
        X_train_1, Y_train_1, X_val_1, Y_val_1 = self.X_train_1, self.Y_train_1, self.X_val_1, self.Y_val_1 
        X_train_2, Y_train_2, X_val_2, Y_val_2 = self.X_train_2, self.Y_train_2, self.X_val_2, self.Y_val_2 
        X_train_3, Y_train_3, X_val_3, Y_val_3 = self.X_train_3, self.Y_train_3, self.X_val_3, self.Y_val_3
        ...
        Result_Path = r"/home/de532237/cancer/Bachelorarbeit/WP2/Methods/feature_analyse/" + Model_Name + '/'
        ...
        if i!=0:
            model = unet_vanilla_fa(input_size=(image_width, image_height, 1), base=2, trainable=True)
            model.compile(optimizer=optimizer, loss=loss, metrics=metrics)
            Pretrained_Weight_Path = self.Pretrained_Weight_Path
            model.load_weights(Pretrained_Weight_Path)
            model.summary()
        else:
            model = unet_vanilla(input_size=(image_width, image_height, 1), base=2,  trainable=True)
            model.compile(optimizer=optimizer, loss=loss, metrics=metrics)
            model.summary()
            pass
...
if __name__ == '__main__' :
    ...
    for i in range (1,4):
        p = Process(target = dataset.feature_analyse, args = (i,))
        p.start()
        p.join() 
        time.sleep(1) 
