...
def unet_vanilla_fa(input_size=(512, 512, 1), base=2, uncertainty=False, trainable=True):
    ...
    u6 = Conv2DTranspose(2 ** (b + 3), (2, 2), strides=(2, 2), padding='same', trainable=trainable)(c5)
    u6 = concatenate([u6, c4])
    c6 = Conv2D(2 ** (b + 3), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(u6) # 22
    c6 = Dropout(0.2)(c6, training=uncertainty)
    c6 = Conv2D(2 ** (b + 3), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(c6) # 24

    u7 = Conv2DTranspose(2 ** (b + 2), (2, 2), strides=(2, 2), padding='same', trainable=trainable)(c6)                          # 25
    u7 = concatenate([u7, c3])
    c7 = Conv2D(2 ** (b + 2), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(u7) # 27
    c7 = Dropout(0.2)(c7, training=uncertainty)
    c7 = Conv2D(2 ** (b + 2), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(c7) # 29

    u8 = Conv2DTranspose(2 ** (b + 1), (2, 2), strides=(2, 2), padding='same', trainable=trainable)(c7)
    u8 = concatenate([u8, c2])
    c8 = Conv2D(2 ** (b + 1), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(u8)                     # 32
    c8 = Dropout(0.1)(c8, training=uncertainty)
    c8 = Conv2D(2 ** (b + 1), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(c8)                     # 34

    u9 = Conv2DTranspose(2 ** b, (2, 2), strides=(2, 2), padding='same', trainable=trainable)(c8)                                                    # 35
    u9 = concatenate([u9, c1], axis=3)
    c9 = Conv2D(2 ** b, (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(u9)                           # 37
    c9 = Dropout(0.1)(c9, training=uncertainty)
    c9 = Conv2D(2 ** b, (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(c9)                           # 39
