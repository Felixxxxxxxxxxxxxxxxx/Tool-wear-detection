...
def unet_vanilla_fe(input_size=(512, 512, 1), base=2, uncertainty=False, trainable=True):

    b = base
    inputs = Input(input_size)

    c1 = Conv2D(2 ** b, (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(inputs)
    c1 = Dropout(0.1)(c1, training=uncertainty)
    c1 = Conv2D(2 ** b, (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(c1)
    p1 = MaxPooling2D((2, 2), padding='same')(c1)

    c2 = Conv2D(2 ** (b + 1), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(p1)
    c2 = Dropout(0.1)(c2, training=uncertainty)
    c2 = Conv2D(2 ** (b + 1), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(c2)
    p2 = MaxPooling2D((2, 2), padding='same')(c2)

    c3 = Conv2D(2 ** (b + 2), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(p2) # 9
    c3 = Dropout(0.2)(c3, training=uncertainty)
    c3 = Conv2D(2 ** (b + 2), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(c3) # 11
    p3 = MaxPooling2D((2, 2), padding='same')(c3)

    c4 = Conv2D(2 ** (b + 3), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(p3) # 13
    c4 = Dropout(0.2)(c4, training=uncertainty)
    c4 = Conv2D(2 ** (b + 3), (3, 3), activation='elu', kernel_initializer='he_normal', padding='same', trainable= not trainable)(c4) # 15
    p4 = MaxPooling2D(pool_size=(2, 2))(c4)
