...
def recall_ini_prediction(y_true, y_pred):
    tp = K.sum(y_true[ :, :, 1] * y_pred[ :, :, 1])
    tp_plus_fn = K.sum(y_true[ :, :, 1]) 
    recall = K.mean((tp) / (tp_plus_fn))
    return (recall)
...
def precision_ini_prediction(y_true, y_pred):
    tp = K.sum(y_true[ :, :, 1] * y_pred[ :, :, 1])
    tp_plus_fp = K.sum(y_pred[ :, :, 1]) 
    precision = K.mean((tp) / (tp_plus_fp))
    return (precision)
...
def F1_score_ini_prediction(y_true, y_pred):
    F1_score = 2*recall_ini_prediction(y_true, y_pred)*precision_ini_prediction(y_true, y_pred)/(recall_ini_prediction(y_true, y_pred)+precision_ini_prediction(y_true, y_pred))
    return F1_score
...
def predict(X,Y,prediction, overlay_with_label=True, overlay_with_info=True):
    ...
    RECALL_INI = recall_ini_prediction(Y,prediction).numpy()
    PRECISION_INI = precision_ini_prediction(Y,prediction).numpy()
    F1_SCORE_INI = F1_score_ini_prediction(Y, prediction)
